...
export function CommentModal({ 
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 md:p-10" onClick={onClose}>
      <div 
        className="bg-background rounded-2xl max-w-6xl w-full h-full md:h-auto md:max-h-[90vh] flex flex-col md:flex-row overflow-hidden shadow-2xl relative"
        onClick={(e) => e.stopPropagation()}
      >
        {/* CLOSE BUTTON OVERLAY */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 bg-black/10 hover:bg-black/20 dark:bg-white/10 dark:hover:bg-white/20 text-gray-500 dark:text-gray-400 rounded-full transition-colors z-50"
        >
          <X className="w-5 h-5" />
        </button>
